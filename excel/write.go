...
func (e *Excel) InitExcel() (err error) {
	...
	// 设置导出文件的文件名
	timeDesc := time.Now().Format("20060102150405")
	fileName := ""
...
func (e *Excel) SetSheet(useStreamWriter bool) error {
	...
	for k, v := range e.exportInfo.Sheets {
		...
		for columnsKey, columnsValue := range v.Columns {
			column, _ := excelize.ColumnNumberToName(columnsKey + 1)
			// 设置标题
			_ = streamWriter.SetRow(column+"1", []interface{}{excelize.Cell{StyleID: headerStyleID, Value: columnsValue.Title}})
		}
		// 保存迭代器
...
func (e *Excel) FillContent(sheetIndex int, dataList []RowData, writeIndex int) (err error) {
	...
	for k, v := range dataList {
		fromIndex := strconv.Itoa(k + writeIndex)
		// 获取当前行样式
		styleID := v.StyleID
		if styleID == 0 {
			styleID = e.getContentStyle()
		}
		for key, value := range v.InterfaceList {
			columnName, _ := excelize.ColumnNumberToName(key + 1)
			fromCell := columnName + fromIndex
			if err = e.exportInfo.Sheets[sheetIndex].StreamWriter.SetRow(fromCell, []interface{}{
				excelize.Cell{StyleID: styleID, Value: value}}); err != nil {
				return
			}
		}
	}
...
func (e *Excel) FillAllContent(sheetIndex int, dataList []RowData) error {
	if len(dataList) <= 0 {
		return nil
	}
	// 防止range out of index
	...
	// 设置表头和行样式
	for k, v := range e.exportInfo.Sheets[sheetIndex].Columns {
		column, _ := excelize.ColumnNumberToName(k + 1)
		// 设置标题
		_ = e.File.SetCellValue(sheetName, column+"1", v.Title)
	}
	headerStyleID := e.GetHeaderStyle()
	...
	for k, v := range dataList {
		fromIndex := strconv.Itoa(k + 2)
		fromCell := "A" + fromIndex
		// 按行赋值
		err := e.File.SetSheetRow(sheetName, fromCell, v.InterfaceList)
		if err != nil {
			return err
		}
		if v.StyleID != 0 {
			// 设置内容样式
			_ = e.File.SetCellStyle(sheetName, fromCell, lastColumn+"fromIndex", v.StyleID)
		}
	}
